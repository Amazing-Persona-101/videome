...
import { DEFAULT_GROUP_NAME, DEFAULT_GROUP_SUMMARY, USE_CANNED_RTK_RESPONSES } from '$lib/constants';
...
export const load = (async ({ fetch }) => {
  ...
  if (!USE_CANNED_RTK_RESPONSES) {
    ...
    filteredSessions = removeEntriesByIds(sessions, idsToRemove);
    ...
    for (const session of filteredSessions) {
      ...
      session.details = {
        group: {
          name: groupDetails?.name || DEFAULT_GROUP_NAME as string,
  ...
  // Log the filtered sessions with groupId and summary
  //console.log('Filtered sessions with groupId and summary:', filteredSessions);
